...
report 50248 "XV Fatt. Accompagnatoria Cust"
{
    Caption = 'XV Fattura Accompagnatoria Custom';
    EnableHyperlinks = true;
    Permissions = TableData "Sales Shipment Buffer" = rimd;
    PreviewMode = PrintLayout;
    WordMergeDataItem = Header;
    RDLCLayout = './ReportLayouts/XVFatturaAccompagnatoriaCustom.rdl';
    ...
    dataset
    {
        dataitem(Header; "Sales Invoice Header")
        {
            ...
            // Campo grezzo (non ricalcolato/compattato da Format Address) per evitare la perdita della riga con alcuni paesi
            column(ShipToAddress2Raw; "Ship-to Address 2")
            {
            }
            ...
            */
            ...
            dataitem(Line; "Sales Invoice Line")
            {
                ...
                column(Description_Line; LineDescriptionForPrint)
                {
                }
                column(ReferenceComment_Line; ReferenceCommentText)
                {
                }
                ...
                begin
                    ...
                    LineDescriptionForPrint := Description;
                    ReferenceCommentText := '';
                    if Type = Type::" " then begin
                        OrderNoFromComment := ExtractOrderNoFromComment(Description);
                        if OrderNoFromComment <> '' then begin
                            VsRifSuffix := GetVsRifSuffix(OrderNoFromComment);
                            if VsRifSuffix <> '' then
                                ReferenceCommentText := VsRifSuffix;
                        end;
                    end;
    ...
    var
        /* RPCustom */
        IsForeign: Boolean;
        LineDescriptionForPrint: Text;
        ReferenceCommentText: Text;
...
#pragma warning restore AA0074
    ...
    // Estrae il numero ordine dalla riga commento "Nr. ordine <No.>" o "Ordine Nr. <No.>" senza toccare il dato salvato
    local procedure ExtractOrderNoFromComment(CommentText: Text[100]): Code[20]
    var
        Marker: Text[20];
        StartPos: Integer;
        Candidate: Text[50];
    ...
    begin
        Marker := 'Order ';
        StartPos := StrPos(CommentText, Marker);
        if StartPos > 0 then begin
            Candidate := CopyStr(CommentText, StartPos + StrLen(Marker));
            StartPos := StrPos(Candidate, ' dated ');
            if StartPos > 0 then
                Candidate := CopyStr(Candidate, 1, StartPos - 1);

            exit(CopyStr(DelChr(Candidate, '<', ' '), 1, 20));
        end;
        ...
        Candidate := DelChr(CopyStr(CommentText, StartPos + StrLen(Marker)), '<', ' ');
        Result := '';
        for i := 1 to StrLen(Candidate) do begin
            CharAt := Candidate[i];
            if not (CharAt in ['A' .. 'Z', 'a' .. 'z', '0' .. '9', '_', '-']) then
                break;
            Result += Format(CharAt);
        end;
        exit(CopyStr(Result, 1, 20));
    end;

    // Recupera "Vs. Rif." e data ordine dall'Ordine collegato (o dal suo archivio se non più esistente)
    local procedure GetVsRifSuffix(OrderNo: Code[20]): Text
    var
    ...
    begin
        ...
        YourRef := '';
        Clear(OrderDate);
        if SalesHeaderOrder.Get(SalesHeaderOrder."Document Type"::Order, OrderNo) then begin
            YourRef := SalesHeaderOrder."Your Reference";
            OrderDate := SalesHeaderOrder."Order Date";
        end else begin
            SalesHeaderArchive.SetRange("Document Type", SalesHeaderArchive."Document Type"::Order);
            SalesHeaderArchive.SetRange("No.", OrderNo);
            if SalesHeaderArchive.FindLast() then begin
                YourRef := SalesHeaderArchive."Your Reference";
        ...
        exit('Vs. Rif. ' + YourRef + ' DATED ' + Format(OrderDate, 0, '<Day,2>/<Month,2>/<Year4>'));
    end;
